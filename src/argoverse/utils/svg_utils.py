...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        hp=helper[0][:20]
#         hp=np.concatenate([hp,[hp[-1]+i*(hp[-1]-hp[-2]) for i in range(1,16)]])
        ############################# find lanes
        world_to_image_space=self.mf.get_candidate_centerlines_for_trajectory(
                        hp,
                        yaw_deg=helper[5],
        ...
        raster_img,cnt_lines_norm=self.mf.get_candidate_centerlines_for_raster(
                        hp,
                        yaw_deg=helper[5],
                        city_name=helper[1][0],avm=self.avm,
            viz=True,
        ...
        cnt_lines_norm=cnt_lines_norm[:MAX_dis_lines]
        #############################
        ...
        if self.use_scene and len(cnt_lines_norm)>0:
...
#         print(agents_num)
        ...
        return {"history_positions": torch.FloatTensor(traj[:self.args.obs_len]),
                ...
                "base_image":raster_img.transpose(2, 0, 1),
...
#         print(seq_path)
        ...
        for group_name, group_data in frames:
            ...
            curr_dist=np.linalg.norm(centroid-cor_xy[-1])
            if curr_dist>MIN_AGENTS_DIST:
                continue
            track_dists.append((group_data,curr_dist))
            
        track_dists.sort(key=operator.itemgetter(1))
...
#         print(len(frames))
        ...
        for group_data,_ in track_dists:
            object_type = group_data["OBJECT_TYPE"].values[0]
